...
class PrefixExpressionTree {
	...
	private int nodesAtLevel(TreeNode node, int desiredLevel, int presentLevel) {
		if (node == null) {
			return 0;
		}
		if (presentLevel == desiredLevel) {
			return 1;
		}
		return nodesAtLevel(node.left, desiredLevel, presentLevel + 1)
				+ nodesAtLevel(node.right, desiredLevel, presentLevel + 1);
	}
