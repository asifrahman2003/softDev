...
// Model objects
  ...
  private class ButtonListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent event) {
      ...
      if (button == restartButton) {
        ...
        throw new IllegalArgumentException("Some button clicked that is not being handled");
      ...
      if (option == 0) {
        ...
        do {
          ...
          newQuestion = JOptionPane.showInputDialog(null,
              "Give me a yes answer for your '" + newAnswer + "'\nthat is also a no question for '" + oldAnswer + "'");
  ...
  private void runSaveGameDialog() {
    if (couldBeSaved) {
      int option = JOptionPane.showConfirmDialog(null, "Overwrite old version with\nyour new question and answers?");
      if (option == 0) {
        theGame.saveGame();
        couldBeSaved = false;
      } else
        JOptionPane.showMessageDialog(null, "Any new questions and answers will not be saved.");
    } else
